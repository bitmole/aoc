...
def extract_digits(s):
    ...
    # map positions of both spelled and literal digits
    for spelled, literal in digit_map.items():
...
def filter_calibration(s):
    ...
    # make number from first and last digit
    first, last = digits[:1], digits[-1:]
    return int(first + last)

def main():
    total = sum([filter_calibration(s) for s in input.splitlines()])
    print('total: ', total)
